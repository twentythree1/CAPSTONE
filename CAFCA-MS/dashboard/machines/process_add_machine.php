...
if (empty($name) || empty($acquisition_date)) {
...
if (!empty($unavailable_from) && !empty($unavailable_until)) {
    ...
    $sql = "INSERT INTO machines (name, status, acquisition_date, unavailable_from, unavailable_until) 
            VALUES ('$name', '$status', '$acquisition_date', '$unavailable_from', '$unavailable_until')";
} elseif (!empty($unavailable_from)) {
    $unavailable_from = $conn->real_escape_string($unavailable_from);
    
    $sql = "INSERT INTO machines (name, status, acquisition_date, unavailable_from) 
            VALUES ('$name', '$status', '$acquisition_date', '$unavailable_from')";
} elseif (!empty($unavailable_until)) {
    $unavailable_until = $conn->real_escape_string($unavailable_until);
    
    $sql = "INSERT INTO machines (name, status, acquisition_date, unavailable_until) 
            VALUES ('$name', '$status', '$acquisition_date', '$unavailable_until')";
} else {
    $sql = "INSERT INTO machines (name, status, acquisition_date) 
            VALUES ('$name', '$status', '$acquisition_date')";
}
